...
describe('Lending contract test cases', function () {
  ...
  let weth, fWeth, dai, fDai, lending, fft;
  ...
  it('1. deploying tokens and lending', async function () {
    ...
    fft = await ERC20.deploy('FFT Token', 'FFT');
  ...
  it('2. checking balances and transfering some tokens', async function () {
    expect(bigToDecimal(await fft.balanceOf(owner.address))).to.equal('100000.0')
    await weth.transfer(user1.address, decimalToBig('10.0'));
    // await weth.transfer(lending.address, decimalToBig('10'));
    await fft.transfer(lending.address, decimalToBig('1000'));

    expect(bigToDecimal(await weth.balanceOf(user1.address))).to.equal("10.0")

    expect(bigToDecimal(await fft.balanceOf(lending.address))).to.equal("1000.0")
  ...
  it('3. lending test', async function () {
    ...
    expect(bigToDecimal(await fft.balanceOf(owner.address))).to.equal('99000.0');

    await weth.approve(lending.address, decimalToBig('100'))
    let amount = decimalToBig('100');
    let days = '2';
    let fftPrice = "1"
    let fftAggre="0x0000000000000000000000000000000000000000";
    let tokenAggre ="0x0000000000000000000000000000000000000000";
    await lending.lend(wethSymbol, amount, days, weth.address, fft.address,fftAggre,tokenAggre);

    expect(bigToDecimal(await weth.balanceOf(lending.address))).to.equal("100.0");
    expect(bigToDecimal(await weth.balanceOf(owner.address))).to.equal("99890.0");
